...
- [ ] 3.3 Add failing real pipeline test `services/extractor/tests/command_pipeline.rs::completed_html_persists_all_candidate_decisions`; assert one command records three scored candidates and exactly one selected strategy matching emitted Document provenance. Run it and confirm no candidate rows exist.
- [ ] 3.4 Record every decision before completion, map quality rejection to a bounded failed-run class, make 3.3 pass, run service format and Clippy gates, and commit this TDD pair on `main`.
