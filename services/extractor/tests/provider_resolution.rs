...
async fn hn_link_post_completes_from_resolved_article() -> Result<(), Box<dyn std::error::Error>> {
    // Server order matters: the Algolia payload embeds the article URL, so the article
    ...
    let algolia_payload = json!({
        "id": 1000,
        "title": "Fixture link post",
        "text": null,
        "url": article_url,
        "children": [
            {
                "id": 1001,
                "text": fixture_comment(DISCUSSION_MARKER_A),
                "children": [],
            },
            {
                "id": 1002,
                "text": fixture_comment(DISCUSSION_MARKER_B),
                "children": [],
            },
        ],
    });
    ...
    // The mapped address keeps the production host shape, so the hermetic test drives the
    // provider boundary directly with its loopback equivalent.
    ...
    // Baseline: the run must still complete end to end regardless of resolution.
    let baseline: (String, Option<String>, i64, i64, i64) = sqlx::query_as(
        "select
            r.status,
            r.last_error_class,
            (select count(*) from extractor.candidates c where c.run_id = r.run_id
                and c.selected and c.strategy = 'hacker_news_item'),
    ...
    // The resolved article must be fetched through the ordinary path, alongside the
    // provider payload.
    let (fetch_total, resolved_fetches): (i64, i64) = sqlx::query_as(
        "select count(*),
                count(*) filter (where f.final_url = $2)
           from extractor.fetches f where f.run_id = $1",
    )
    .bind(run.run_id)
    .bind(&article_url)
    .fetch_one(pool)
    ...
    // Every resolution step must be persisted in order.
    let resolutions: Vec<(i32, String, Option<String>)> = sqlx::query_as(
        "select ordinal, kind, resolved_url from extractor.provider_resolutions
    ...
    // Both payloads stay reprocessable as raw source blobs.
    ...
    // The stored document must be the external article, not the discussion conversion.
    ...
    let ir_ref: BlobRef = serde_json::from_value(json!({
        "owner_service": ir_row.0,
        "digest": {"algorithm": ir_row.1, "hex": ir_row.2},
        "media_type": ir_row.3,
        "length_bytes": u64::try_from(ir_row.4)?,
    }))?;
